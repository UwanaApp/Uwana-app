...
const Post = ({ post }) => {
  ...
  return (
    <div className="timeline-post border rounded-lg shadow-lg  mb-6">
      <div className="post-header flex items-center px-3 py-4  justify-between ">
        <div className="flex items-center">
          <img
            className="profile-picture w-12 h-12 rounded-full mr-4"
        ...
        {/* Meat Menu */}
        <div className="meat-menu md:hidden lg:hidden ml-auto">
      ...
      <div className="post-images mb-4 ">
        {post.mediaType === "video" ? (
          <video className="post-video w-full  object-cover" controls>
            <source src={post.media} type="video/mp4" />
            <source
              src={post.media.replace(".mp4", ".webm")}
              type="video/webm"
            />
      ...
      <div className="post-content px-3 py-0 ">
      ...
      <div className="reaction px-4 py-2 flex items-center justify-between">
        <div className="emoji-reactions flex space-x-6">
          {/* Love Emoji */}
          <div
            className={`emoji flex items-center space-x-1 cursor-pointer transform transition-transform duration-200 ${
              selectedEmoji === "love" ? "scale-125" : ""
            }`}
          ...
          <div
            className={`emoji flex items-center space-x-1 cursor-pointer transform transition-transform duration-200 ${
              selectedEmoji === "twitter" ? "scale-125" : ""
            }`}
            onClick={() => handleEmojiClick("twito")}
          >
            <img
              className={`w-6 h-6 ${
                selectedEmoji === "twitter" ? "opacity-100" : "opacity-80git"
              }`}
          ...
          <div
            className={`emoji flex items-center space-x-1 cursor-pointer transform transition-transform duration-200 ${
              selectedEmoji === "star" ? "scale-125" : ""
            }`}
            onClick={() => handleEmojiClick("star")}
          >
            <img
              className={`w-6 h-6 ${
                selectedEmoji === "star" ? "opacity-100" : "opacity-80"
              }`}
